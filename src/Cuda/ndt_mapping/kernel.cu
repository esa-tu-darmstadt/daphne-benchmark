...
int target_cells_size_;
__device__ __managed__ int voxelDimension[3];

__device__ __managed__ PointXYZI minVoxel;
__device__ __managed__ PointXYZI maxVoxel;
__device__  const float resolution_ = 1.0;
...
int ndt_mapping::read_next_testcases(int count)
{
	...
	cudaFree(maps[i]);
    delete [] maps;
    maps = new PointCloudArray[count];
    delete [] maps_size_;
    maps_size_ = new int[count];
...
__device__ int linearizeAddr(const int x, const int y, const int z)
{
    return  (x + voxelDimension[0] * (y + voxelDimension[1] * z));
}

__device__ int linearizeCoord(const float x, const float y, const float z)
{
...
__device__ void invertMatrix(Mat33 &m)
{
    Mat33 temp;
...
/**
   Performs init of the voxel grid: finding min/max values 
*/	   
__global__ void initComputeMinMax(PointCloudArray target_, int size)
{
  int thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  
  PointXYZI min = {0.0, 0.0, 0.0, 0.0 };
  PointXYZI max = {0.0, 0.0, 0.0, 0.0 };

  __shared__ PointXYZI ex[512];
  
  for (int i = thread_id; i < size; i+=blockDim.x)  
    for (int elem = 0; elem < 3; elem++)
      {
	if (target_[i].data[elem] > max.data[elem])
	  max.data[elem] = target_[i].data[elem];
	if (target_[i].data[elem] < min.data[elem])
	  min.data[elem] = target_[i].data[elem];
      }
  ...
  if (thread_id == 0)
	...
	for (int elem = 0; elem < 3; elem++)
	  {
	    if (ex[i].data[elem] < min.data[elem])
	      min.data[elem] = ex[i].data[elem];
	  }
      minVoxel = min;
    }
  ...
  if (thread_id == 0)
	...
	for (int elem = 0; elem < 3; elem++)
	  {
	    if (ex[i].data[elem] > max.data[elem])
	      max.data[elem] = ex[i].data[elem];
	  }
      maxVoxel = max;
    }  
}

__global__ void initComputeInit(TargetGridLeadConstPtr *target_cells_, int size)
{
  ...
  target_cells_[id].numberPoints = 0;
  target_cells_[id].mean[0] = 0;
  target_cells_[id].mean[1] = 0;
  target_cells_[id].mean[2] = 0;
  memset(target_cells_[id].invCovariance.data, 0, sizeof(double) * 3 * 3);
  target_cells_[id].invCovariance.data[2][0] = 1.0;
  target_cells_[id].invCovariance.data[1][1] = 1.0;
  target_cells_[id].invCovariance.data[0][2] = 1.0;
}

__global__ void initComputeStep1(PointCloudArray target_, TargetGridLeadConstPtr *target_cells_, int size)
{
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= size)
    return;
  ...
  atomicAdd(&target_cells_[voxelIndex].mean[0], (double)(target_)[id].data[0]);
  atomicAdd(&target_cells_[voxelIndex].mean[1], (double)(target_)[id].data[1]);
  atomicAdd(&target_cells_[voxelIndex].mean[2], (double)(target_)[id].data[2]);
  atomicAdd(&target_cells_[voxelIndex].numberPoints, 1);
  
  // sum up x * xT for single pass covariance calculation
  for (int row = 0; row < 3; row ++)
    for (int col = 0; col < 3; col ++)
      atomicAdd( &target_cells_[voxelIndex].invCovariance.data[row][col], (double)(target_)[id].data[row] * (target_)[id].data[col]);

}

__global__ void initComputeStep2(TargetGridLeadConstPtr *target_cells_, int size)
{
  int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= size)
    return;
  ...
  target_cells_[id].mean[1] /= target_cells_[id].numberPoints;
  target_cells_[id].mean[2] /= target_cells_[id].numberPoints;
  ...
  for (int row = 0; row < 3; row++)
    for (int col = 0; col < 3; col++)
      {
	target_cells_[id].invCovariance.data[row][col] = (target_cells_[id].invCovariance.data[row][col] -
							 2 * (pointSum[row] * target_cells_[id].mean[col])) / size +
	  target_cells_[id].mean[row]*target_cells_[id].mean[col];
	target_cells_[id].invCovariance.data[row][col] *= (size -1.0) / target_cells_[id].numberPoints; 
      }
  ...
  invertMatrix(target_cells_[id].invCovariance);
...
void ndt_mapping::initCompute()
{
    ...
    target_cells_size_ = voxelDimension[0] * voxelDimension[1] * voxelDimension[2];
    cudaMallocManaged(&target_cells_, sizeof(TargetGridLeadConstPtr) * target_cells_size_);
    dim3 threaddim2(THREADS);
    dim3 blockdim2((target_cells_size_+THREADS-1)/THREADS); // round up, so we dont miss one
    for (int i = 0; i < target_cells_size_; i++)
	{
    ...
    dim3 threaddim1(THREADS);
    dim3 blockdim1((target_size_+THREADS-1)/THREADS); // round up, so we dont miss one
    //    initComputeMinMax<<<1, threaddim1>>>(target_, target_size_);
    ...
    dim3 threaddim2(THREADS);
    dim3 blockdim2((target_cells_size_+THREADS-1)/THREADS); // round up, so we dont miss one
    initComputeInit<<<blockdim2, threaddim2>>>(target_cells_, target_cells_size_);
    cudaDeviceSynchronize();
    */

    // first pass to put everything in the right voxel leaf
    initComputeStep1<<<blockdim1, threaddim1>>>(target_, target_cells_, target_size_);
    cudaDeviceSynchronize();
    
    // second pass to finalize average and sum of all leafs
    initComputeStep2<<<blockdim2, threaddim2>>>(target_cells_, target_cells_size_);
    cudaDeviceSynchronize();
}

void ndt_mapping::deinitCompute()
{
  cudaFree(target_cells_);
}
...
inline double ndt_mapping::findNearest(PointXYZI &p)
{
    ...
    double dist = 0.0;
    for (size_t j = 0; j < target_size_; j++)
	{
...
// final_transformation_ is get and and converged_ is checked
 CallbackResult ndt_mapping::partial_points_callback(PointCloud &input_cloud, Matrix4f &init_guess, PointCloudArray target_cloud, int target_cloud_size)
{
    CallbackResult result;

    input_ = &input_cloud;
    target_ = target_cloud;
    target_size_ = target_cloud_size;
...
void ndt_mapping::run(int p) {
    ...
    while (read_testcases < testcases)
	{
	    ...
	    for (int i = 0; i < count; i++)
		{
		  ...
		  results[i] = partial_points_callback(filtered_scan_ptr[i], init_guess[i], maps[i], maps_size_[i]);
		}
	    pause_func();
