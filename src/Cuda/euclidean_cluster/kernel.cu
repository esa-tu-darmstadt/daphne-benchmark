...
*/
__global__ void initRadiusSearch(const Point* __restrict__ points, int number_points, bool* __restrict__ sqr_distances, double radius)
{
  int n = number_points;
  // 16 byte aligned memory accesses (128 Bit memory interface)
  ...
  for (int i = 0; i < n; i++)
	...
	int array_index = i * aligned_length + j;
...
__global__ void parallelRadiusSearch(const int* __restrict__ point_index, int start_index, int search_points, bool* __restrict__ indices, const bool* __restrict__ sqr_distances, int cloud_size)
{
	...
	int array_index = point_index[search_point_index] * aligned_length +id;
			  ...
			  unsigned int max_pts_per_cluster)
{
  ...
  dim3 blockdim((cloud_size+THREADS-1)/THREADS); // round up, so we dont miss one
  cudaMallocManaged(&sqr_distances, (cloud_size * (cloud_size + 16) * sizeof(bool)));
  initRadiusSearch<<<blockdim, threaddim>>>(cloud, cloud_size, sqr_distances, tolerance);
  ...
  for (int i = 0; i < static_cast<int> (cloud_size); ++i)
  {
	...
	{
	  parallelRadiusSearch<<<blockdim, threaddim>>>(seed_queue, queue_last_element - new_elements, queue_last_element, nn_indices, sqr_distances, cloud_size);
          new_elements = 0;
	  cudaDeviceSynchronize();
...
void
extract (const PointCloud input_, int cloud_size, std::vector<PointIndices> &clusters, double cluster_tolerance_)
{
    if (cloud_size == 0)
	{
	    clusters.clear ();
...
*/
void euclidean_clustering::segmentByDistance(PointCloud in_cloud_ptr,
					     int cloud_size,
                PointCloudRGB *out_cloud_ptr,
                BoundingboxArray *in_out_boundingbox_array,
                Centroid *in_out_centroids,
                double in_max_cluster_distance=0.5)
{
	...
	int current_segment_pos[5] = { 0, segment_size[0], segment_size[0]+segment_size[1], segment_size[0]+segment_size[1]+segment_size[2],
...
// implemented for comparing, so we can make a normalized order, using plain lexicographical order
inline bool 
