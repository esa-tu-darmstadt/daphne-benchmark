...
int radiusSearch(const int point_index, std::vector<int> & indices, const bool* sqr_distances, int total_points)
{
    ...
    // all points with a smaller index
    ...
    // the point itself has distance 0, but is not considered to be one of its neighbours, so its not put in
    // distance.push(point_index);

    // all points with a higher index
			  ...
			  unsigned int max_pts_per_cluster)
{
  ...
  processed = (bool*) malloc(sizeof(bool) * cloud.size());
