	...
	if (iVoxel < gridInfo->gridSize) {
		...
		for (int i = 0; i < EPHOS_VOXEL_POINT_STORAGE && i < voxel.pointStorageLevel; i++) {
			for (char row = 0; row < 3; row++) {
				...
				voxel.mean[row] += voxel.pointStorage[i].data[row];
				for (char col = 0; col < 3; col++) {
		...
		double pointSum[3];
		for (char k = 0; k < 3; k++) {
			pointSum[k] = voxel.mean[k];
			voxel.mean[k] /= pointNo;
		}
		// postprocess the matrix
		for (char row = 0; row < 3; row++) {
			for (char col = 0; col < 3; col++) {
				double tmp = (voxel.invCovariance.data[row][col] - 2 *
					(pointSum[row] * voxel.mean[col])) /
 					gridInfo->gridSize + voxel.mean[row]*voxel.mean[col];
